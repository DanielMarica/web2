...
router.get("/", (req, res) => {
  ...
  if (isNaN(minDuration) || minDuration <= 0) {
    res.sendStatus(400).json("Wrong minimum duration"); // bad practice (will be improved in exercise 1.5)
  }
...
router.get("/:id", (req, res) => {
  ...
  if (isNaN(id)) {
    return res.sendStatus(400).json("Wrong minimum duration"); // bad practice (will be improved in exercise 1.5)
  }

  const film = films.find((film) => film.id === id);

  if (film === undefined) {
    return res.sendStatus(404).json("Resource not found"); // bad practice (will be improved in exercise 1.5)
  }
...
router.post("/", (req, res) => {
  ...
  if (
    ...
    return res.sendStatus(400).json("Wrong body format"); // bad practice (will be improved in exercise 1.5)
  }
